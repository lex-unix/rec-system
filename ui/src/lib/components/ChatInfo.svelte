...
<div use:melt={$root} class="w-[420px] max-w-xs">
	<div
		use:melt={$list}
		class="mb-10 flex items-center justify-between overflow-x-auto rounded-md border border-navy-200/10 bg-navy-900 p-1"
		aria-label="Manage your account"
	>
		{#each triggers as triggerItem}
			<button
				use:melt={$trigger(triggerItem.id)}
				class="relative h-10 flex-1 select-none rounded px-4 py-3 leading-none text-navy-400 transition-colors duration-200 focus:outline-none data-[state=active]:text-navy-50"
			>
